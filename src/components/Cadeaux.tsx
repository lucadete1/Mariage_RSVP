...
export default function Cadeaux() {
  ...
  return (
    <section id="cadeaux" style={{ padding: '6rem 2rem', background: 'var(--cream)' }}>
      ...
      <div className="fade-in-up" style={{ maxWidth: 900, margin: '0 auto 2rem', overflowX: 'auto' }}>
          ...
          <tbody>
            {GIFTS.map(([n, desc, qty], i) => (
              <tr key={i} style={{ borderBottom: '1px solid rgba(201,169,110,0.2)' }}>
                <td style={{ padding: '0.75rem 1rem', color: 'var(--text-muted)', fontSize: '0.8rem', verticalAlign: 'top' }}>{n}</td>
                <td style={{ padding: '0.75rem 1rem', color: 'var(--text)', verticalAlign: 'top' }}>{desc}</td>
                <td style={{ padding: '0.75rem 1rem', color: 'var(--text)', verticalAlign: 'top' }}>{qty}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="fade-in-up" style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem' }}>
        <button onClick={generateGiftsPdf}
          style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontFamily: 'var(--font-sans)', fontSize: '0.78rem', fontWeight: 500, letterSpacing: '0.15em', textTransform: 'uppercase', background: 'var(--gold)', color: '#fff', padding: '0.85rem 2rem', textDecoration: 'none', transition: 'background 0.2s', borderRadius: 2, border: 'none', cursor: 'pointer' }}
